            ...
            if (count($this->Model_Consulta->validacionAct_proy($CodigoUnico)) > 0) {
                ...
                try {
                    ...
                    foreach ($proyecto_snip_nombre_DATA as $itemp) {
                        ...
                        $nombre                = $itemp->nombre;
                        $proyecto_nombre_local = $nombre; //asignando nombre para mostrar
...
//meta
                    ...
                    //$cadena_fil =  eliminar_tildes($proyecto_nombre_local);

                    $data['mensaje'] = 'Proyecto con codigo (' . $CodigoUnico . ') se actualizo correctamente';
                    $data['actualizo'] = true;
                } catch (Exception $e) {
                    $this->db->trans_rollback();
                    $data['mensaje'] = 'ERROR, Proyecto con codigo (' . $CodigoUnico . ') no pudo ser actualizado';
...
//meta
                    ...
                    echo json_encode(['proceso' => 'Correcto', 'mensaje' => '"' . 'HOLA' . '" (' . $CodigoUnico . ') se actualizo correctamente']);exit;
                    log_message('error', 'MENSAJE ERROR');
                } catch (Exception $e) {
                    $this->db->trans_rollback();
